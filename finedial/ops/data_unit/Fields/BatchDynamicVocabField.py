...
class BDVField(RawField):
    ...
    def __init__(self, sequential=True, use_vocab=True, init_token=None,
                 ...
                 is_target=False, dynamic_vocab=False):
    ...
    def process(self, batch, device=None):
        ...
        if self.dynamic_vocab:
            ...
            token_set = set()
            for line in padded[0] if self.include_lengths else padded:  # 把Batch里的所有Token放一起
        ...
        if self.include_lengths:
            ...
            var, lengths = self.numericalize(padded, my_vocab, device=device)
            if self.dynamic_vocab:
                return var, lengths, (uni_token_vocab_t2i, uni_token_vocab_i2t_list)
            else:
                return var, lengths
        else:
            my_vocab = self.vocab.stoi if not self.dynamic_vocab else uni_token_vocab_t2i
    ...
    def numericalize(self, arr, my_vocab, device=None):
        ...
        if self.use_vocab:
            if self.sequential:
                arr = [[my_vocab[x] for x in ex] for ex in arr]
            else:
