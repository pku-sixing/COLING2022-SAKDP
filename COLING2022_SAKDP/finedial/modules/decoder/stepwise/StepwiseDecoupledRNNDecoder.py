...
class StepwiseDecoupledRNNDecoder(StepwiseDecoderBase):

    def __init__(self, params, memory_agents={}):
        super(StepwiseDecoupledRNNDecoder, self).__init__(params)
        ...
        self.predict_with_full_states = params.get('predict_with_full_states', False)
        assert self.predict_with_full_states is False, 'do not implement this now'

        # 配置Memory Agent
        self.memory_readout_dim = 0
        memory_generation_mode_names = []
        ...
        for agent_name in memory_agents.keys():
            ...
            memory_generation_mode_names += agent.get_gen_mode_name()
        ...
        self.memory_generation_mode_names = ['vocab'] + memory_generation_mode_names
        ...
        # 开始构造词表的输出预测
        if len(memory_generation_mode_names) > 0:

            self.generation_mode_selectors = dict()
            self.generation_mode_selector = nn.ModuleDict()
            for order_name in self.memory_generation_mode_names:
                # Orders
                if self.generation_mode_gate_mode == 'mlp':
                    ...
                    selector = nn.Linear(self.state_output_dim, 1,  bias=False)
                else:
                    raise NotImplementedError()
                self.generation_mode_selector[order_name] = selector
        else:
            raise NotImplementedError()
    ...
    def forward(self, last_state, current_input, memory_dict, dynamic_projections):
        ...
        for agent_name in self.memory_agents.keys():
            ...
            assert len(local_copy_prob) == len(agent.get_gen_mode_name())
            local_copy_probs += local_copy_prob


        # Except Vocab Copy Mode
        assert len(local_copy_probs) == len(self.memory_generation_mode_names) - 1

        # 合并
        generation_mode_gate_logits = [self.generation_mode_selector[mode_name](current_dialogue_context)
                                       for mode_name in self.memory_generation_mode_names]
        generation_mode_gate_logits = torch.cat(generation_mode_gate_logits, -1)
        ...
        local_probs = [local_probs[idx] * generation_mode_gate[:, idx:idx + 1]
                       for idx in range(len(self.memory_generation_mode_names))]
